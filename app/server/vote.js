...
var vote = function (req, res) {
    User.findOne({ username : req.session.username }, function(err, user) {
        if (!err && user  && user.likes.indexOf(req.body.torrentId) == -1  && user.dislikes.indexOf(req.body.torrentId) == -1){
                ...
                if (!er && movie) {
                    if (req.body.vote == 'down') {
                        ...
                        Movie.update({'torrent.id':req.body.torrentId}, {$set : {dislike : dislike}}, function (err, doc) {
                            User.update({ username : req.session.username }, {$addToSet: {dislikes: req.body.torrentId}}, function (err, u) {
                        ...
                        Movie.update({'torrent.id':req.body.torrentId}, {$set : {like : like}}, function (err, doc) {
                            User.update({ username : req.session.username }, {$addToSet: {likes: req.body.torrentId}}, function (err, u) {
                                res.json({likes: like});
                            });
                        });
                    } else {
